...
function vbean_error_mailer($errno, $errstr, $errfile, $errline) {
        ...
        if($lasterror['type']==$errno && $lasterror['message']==$errstr&& $lasterror['file']==$errfile&&$lasterror['line']==$errline){
            return;
        }
        ...
        if($exemptionlist!=FALSE){
            ...
            try{
                ...
                foreach($exemptions as $exemption){
                    ...
                    if(($thisrow[0]==$errfile|| \VanillaBeans\vbean_endsWith($errfile, $thisrow))||  \VanillaBeans\vbean_startsWith($thisrow[0],$errstr)){
                        if(count($thisrow)>1){
                            if(is_int($thisrow[1])){
